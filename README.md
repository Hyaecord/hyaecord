...
- **Stoat.chat support**: a real, separate integration talking to Stoat's own public REST/WebSocket API (`api.stoat.chat`, `events.stoat.chat`) — not a guess, verified against Stoat's own published OpenAPI spec and protocol docs. Real and working today: login (embedded stoat.chat page, same trustworthy pattern as Discord's), servers/channels/DMs, real per-server members and presence, friends (including accept/decline/remove), reading and sending messages with live updates, editing/deleting your own messages, reactions, attachments (viewing — sending one isn't built yet), pinned messages, channel search, and typing indicators. Connect an account from Settings → Accounts; a "Merge accounts into one sidebar" toggle shows Discord and Stoat servers together, each with a small platform badge, or lets you view one platform at a time. Not built yet: voice (Stoat's real transport is LiveKit, its own project the same way Discord's real voice transport is), Fluxer support, and sending attachments. See `BUILD_PROMPT.md` for the exact, itemized history of what shipped and why.
...
- **Bundled plugins ported from Equicord/Vencord**: TalkInReverse, WriteUpperCase, Signature, PolishWording (partial), GoogleThat, and SendTimestamps (partial) are real, from-scratch reimplementations of those plugins' behaviour, each shown in Settings with an attribution badge linking back to the original — one logo per project it genuinely ships in (SendTimestamps is Vencord-original and bundled into Equicord, so it shows both; the rest are Equicord-exclusive). See [PLUGIN_PARITY.md](PLUGIN_PARITY.md) for a full audit of Equicord's entire 361-plugin catalogue — what's ported, what's plausible, and what's architecturally impossible and why.
...
| GIF picker                            |    Supported     | Real Discord GIF search (Tenor via Discord's own proxy) when logged into Discord; falls back to a real Giphy search when it isn't (e.g. Stoat-only), via an optional local API key — see `.env.example` |
