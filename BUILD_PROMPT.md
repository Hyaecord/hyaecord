...
- **Moderator View — mass-delete channels (Shift+M)** — **not yet built.** Needs: a permission check against the (not-yet-built) guild/channel permission model, a selection-mode UI overlay, and a "drag selected channels into a trash" animation — owner suggested a hyena-themed spin on it (chewing/dragging-into-den imagery fits the mascot). Blocked on the Discord permissions data not being fetched yet; build after the guild/channel model is fleshed out past the current READY-only shape.
...
- **Theme picker as a searchable gallery** — once the Theme Store (explicitly V2 in this doc) exists, revisit; the current three-theme (Light/Dark/AMOLED) picker in the first-run wizard and Settings doesn't need a gallery/carousel yet.
...
- **"Server Chomper" — swipeable hide/mute for servers and friends** — **not yet built, needs design follow-up.** Swipe/drag a server or DM one direction to hide-and-mute it (not delete), the other direction to keep; a Settings toggle un-mutes/un-hides everything the Chomper touched. Vanilla port of the reference `Stack` drag-to-back interaction: use pointer `drag` deltas + a CSS `transform: translateX() rotate()` with a spring-ish `transition`, no physics lib. Needs a server/DM data model with a `hiddenByChomper: boolean` flag first — not present yet.
...
9. ✅ **Native mention/DM notifications** (`src/main/notifications.ts` + `discord/index.ts` `maybeNotify()`): fires from the *main* process (not the renderer) specifically so it keeps working while the window is minimized or unfocused — libnotify on Linux via Electron's `Notification` API. Triggers only on DMs (`!guild_id`) and direct `@mention`s of the logged-in user, never a blanket new-message ping; suppressed when the window is already focused (no double-notify); clicking restores/focuses the window. This is the "minimal always-on listener for mentions/DMs/calls" half of Gaming Mode from the brief.
   **Honest gap:** the other half — actually detecting that another app went fullscreen/is a game, to throttle Hyaecord's own resource use — is not implemented and is genuinely hard cross-desktop (X11 has `_NET_WM_STATE_FULLSCREEN` via a native call or `wmctrl`; Wayland compositors mostly expose nothing here without portal support, which is exactly why the brief hedges with "⚠ verify"). There's also currently very little background work in this app *to* throttle (no polling loops exist). The Settings toggle (`gamingMode`) is still just a boolean with no behavior wired to it yet — next increment on this should either (a) research what's actually detectable on GNOME/KDE and wire a real detector, or (b) redefine what the toggle does in the meantime (e.g. suppress the notifications just added) rather than leaving it inert.
