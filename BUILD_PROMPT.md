...
- **Dynamic rotating text** — for loading/tooltip copy ("Connecting…" cycling through translated strings or a fun fact rotation): a simple `setInterval` + CSS fade/slide, matches the reference's cadence (~300ms per swap, stops on last item rather than looping forever unless explicitly looped).
- **Particle burst on donate/support click** — small celebratory puff of dots on a "Support the project" button. Vanilla: spawn 6 short-lived absolutely-positioned `div`s from the click point, animate `transform`/`opacity` via the Web Animations API (`el.animate(...)`), remove on `finish`. No React state needed.
...
- **Channel sidebar "proximity" hover effect** — channel names in the sidebar brighten/shift toward the accent colour as the cursor nears them vertically (reference: React Bits `LineSidebar`). Vanilla port: on `pointermove` over the channel `<ul>`, compute each `<li>`'s distance from the cursor and set a `--effect` CSS custom property (0–1) consumed by `color: color-mix(...)` and a `transform: translateX(...)` — no rAF-driven physics required, a single `pointermove` handler with a CSS transition is enough at this scale. Nice-to-have polish, not urgent.
